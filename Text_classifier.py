...
columndrop = ['Unnamed: 2', 'Unnamed: 3', 'Unnamed: 4']
messages = messages.drop(columns=columndrop, axis=1)
...
# Vectorize the text
tfidf = TfidfVectorizer(tokenizer=lambda x: cleantext(x).split())
...
predicted_label = label_encoder.inverse_transform(y_pred)  # Decode the predicted label
